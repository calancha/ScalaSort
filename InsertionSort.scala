// Implement insertiion algorithm
...
object InsertionSort extends App {
  ...
  def getLists(sorted: List[Int], unsorted: List[Int]): (List[Int], List[Int]) = {
    unsorted.isEmpty match {
      ...
      case false =>
        // Remove the 1st element (elt) from unsorted and add it into sorted at the right position
        val elt = unsorted.head
        if (sorted.isEmpty || elt >= sorted.last) {
          getLists(sorted :+ elt, unsorted.tail)
        } else {
          // Insert elt in sorted at the right position
          val splitedSort = sorted.partition(_ < elt)
          getLists(splitedSort._1 ::: elt :: splitedSort._2, unsorted.tail)
        }
    }
  }
  ...
  val testResults = (lists zip results).map (pair => insertionSort(pair._1) == pair._2)
